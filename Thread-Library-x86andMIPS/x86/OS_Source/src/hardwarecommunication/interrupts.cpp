...
uint32_t InterruptManager::DoHandleInterrupt(uint8_t interrupt, uint32_t esp)
{
    ...
    {
        // Memory ile alakalı kısımlarda gelir
        // Thread bitmediği için thread PC memory exception atar
        // Su an memory ile alakalı tek exception threadler ile ilgili
        // Ileride farklı şeyler yapıldığında burası genişletilmeli
        ...
        // Fakat ortada task var ise ve bu bloğa giriyor ise
        // Ben bunu handle ederim
        else {
            esp = (uint32_t)taskManager->Schedule((CPUState*)esp, true);
        }
    }
